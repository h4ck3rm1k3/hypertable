...
void Range::initialize() {
  ...
  htforeach(Schema::AccessGroup *sag, m_schema->get_access_groups()) {
    ...
    htforeach(Schema::ColumnFamily *scf, sag->columns)
...
void Range::update_schema(SchemaPtr &schema) {
  ...
  htforeach(Schema::AccessGroup *s_ag, new_access_groups) {
...
void Range::split_compact_and_shrink() {
  ...
  if (m_split_off_high) {
    /** Create DFS directories for this range **/
    {
      ...
      {
        ScopedLock lock(m_schema_mutex);
        htforeach(Schema::AccessGroup *ag, m_schema->get_access_groups()) {
