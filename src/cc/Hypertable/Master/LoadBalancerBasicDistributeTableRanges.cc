...
void LoadBalancerBasicDistributeTableRanges::compute_plan(vector<RangeServerStatistics> &range_server_stats, BalancePlanPtr &balance_plan) {
...
void LoadBalancerBasicDistributeTableRanges::compute_moves(BalancePlanPtr &plan) {
  ...
  while (scanner->next(cell)) {
    ...
    if (last_location.size() > 0 && read_start_row) {
      ...
      bool added;
      added = maybe_add_to_plan(table.c_str(), last_location.c_str(), last_start_row.c_str(),
                                end_row.c_str(), plan);
      if (added)
        HT_DEBUG_OUT << "Added move for range " << table << ":" << end_row << ", start_row="
                     << last_start_row << ", location=" << last_location << HT_END;
    }
  }
}
...
bool LoadBalancerBasicDistributeTableRanges::maybe_add_to_plan(const char *table,
  ...
  htforeach(const char *target_server, m_servers) {
